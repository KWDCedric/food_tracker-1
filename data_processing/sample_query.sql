...
WITH temp_protein AS(
    SELECT F.fdc_id, FN.amount*(22/100) AS protein_value
    FROM food F JOIN food_nutrient FN ON F.fdc_id = FN.fdc_id
            JOIN nutrient N ON FN.nutrient_id = N.nutrient_id
    WHERE N.name = 'protein' AND F.fdc_id=167536
),
temp_fat AS(
    SELECT F.fdc_id, FN.amount*(22/100) AS fat_value
    FROM food F JOIN food_nutrient FN ON F.fdc_id = FN.fdc_id
            JOIN nutrient N ON FN.nutrient_id = N.nutrient_id
    WHERE N.name = 'carbohydrate, by difference' AND F.fdc_id=167536
),
temp_carb AS(
    SELECT F.fdc_id, FN.amount*(22/100) AS carbohydrate_value
    FROM food F JOIN food_nutrient FN ON F.fdc_id = FN.fdc_id
            JOIN nutrient N ON FN.nutrient_id = N.nutrient_id
    WHERE N.name = 'total lipid (fat)' AND F.fdc_id=167536
)
